...
function dzTree(treeName) {
	...
	this.getIcon2 = function(theNode) {
		...
		if(theNode._lastChildId) {
			if(theNode._isOpen) {
				src = icon.folderOpen;
				src = IMGDIR + '/ico_4.gif'.replace('4',theNode.id); //eachcan add
			} else {
				src = icon.folder;
				src = IMGDIR + '/ico_4.gif'.replace('4',theNode.id); //eachcan add
			}
	...
	this.closeAll = function (){
		...
		for(k in this.nodes)
		{
			if (this.nodes[k] && this.nodes[k].pid == 0)
			{
				this.nodes[k]._isOpen = false;
				$('nodes_' + this.nodes[k].id).style.display = 'none';
				$('icon1_' + this.nodes[k].id).src =  theTree.getIcon1(this.nodes[k]);
				$('icon2_' + this.nodes[k].id).src = theTree.getIcon2(this.nodes[k]);
			}
		}
	}
}